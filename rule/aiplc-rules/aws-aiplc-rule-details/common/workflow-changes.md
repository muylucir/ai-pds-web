...
4. **Update State**: Mark phase as "PENDING" in `aiplc-state.md`
...
5. **Update State**: Mark phase as "SKIPPED" in `aiplc-state.md`
...
3. **If Restart Chosen**:
   ...
   - Mark stage as "IN PROGRESS" in `aiplc-state.md`
...
4. **If Confirmed**:
   ...
   - Reset all affected stages in `aiplc-state.md`
...
3. **Update State**: Ensure `aiplc-state.md` reflects current status
...
**On Resume**:
1. **Detect Existing Project**: Check for `aiplc-state.md`
...
2. **Update All Tracking**: Keep `aiplc-state.md`, plan files, and `audit.md` in sync
