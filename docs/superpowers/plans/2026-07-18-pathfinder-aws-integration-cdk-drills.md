...
8. **Task 5's `harness_factory` calls the blocking `mint_harness_token` directly on the event loop — a documented, not-yet-resolved contradiction.** Task 4's `mint_harness_token` docstring is explicit that it is "a sync, blocking boto3 call ... Task 5's async caller (app.py's `harness_factory`) must wrap this call in `asyncio.to_thread`." But `harness_factory`'s signature (`Callable[[VMHandle], HarnessLike]`, `microvm.py`) is and must remain **sync** — `MicroVMSandbox._ensure_ready`/`_boot_and_restore` call it as a plain (non-awaited) function on every boot/resume/reboot transition, and every existing test (`test_microvm_sandbox.py`, `test_microvm_persistence.py`, `test_microvm_recovery.py`, `test_make_sandbox.py`) constructs it as `harness_factory=lambda handle: harness` — a sync lambda. `to_thread` is unusable from a sync function without `asyncio.run`-ing a coroutine there, which is not what Task 4's comment implies and was never implemented in Task 5. The result actually shipped: `_harness_token_provider` (`app.py`) calls `mint_harness_token(vm_id, region)` directly, so a real (non-`fake-`) handle transition blocks the event loop for the duration of one `CreateMicrovmAuthToken` boto3 call. **Controller decision:** accept this as a documented limitation for now — same class of deferral as Open Question 1's `_map_status` fallback — under the workshop's few-concurrent-tenants model (`docs/superpowers/plans/2026-07-18-pathfinder-microvm-persistence-recovery.md` Open Question 4: "the workshop model is 'session-단위 소수 테넌트' (few concurrent tenants)"); the call is rare (once per handle transition, not per request) and short-lived. **Resolution when concurrency grows:** make `harness_factory` async (`Callable[[VMHandle], Awaitable[HarnessLike]]`), `await`-wrapping `mint_harness_token` in `asyncio.to_thread` inside `_harness_token_provider`; this ripples into `microvm.py`'s `_ensure_ready`/`_boot_and_restore` (both call sites become `await self._harness_factory(...)`) and every test's inline sync lambda (becomes `async def`/awaited or wrapped). Not done here — disproportionate to the drill-stage scope guard (`microvm.py` and `harness_factory`'s signature are explicitly out of scope for Task 5).
