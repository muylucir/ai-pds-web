...
`PrototypeCard`에 "초기화" 버튼. 확인 토스트에 삭제 항목을 나열하고, 설문 응답이
1건 이상이면 갯수를 명시한다("응답 12건이 삭제됩니다"). 응답 갯수는 목록 응답에
이미 없으므로, 확인 시점에 `GET .../survey`로 조회하거나 목록에 필드를 추가한다 —
후자를 택한다(추가 요청 없이 버튼 활성화 시점에 이미 알 수 있다).

성공 후 `list.reload()`로 카드가 `none`으로 갱신된다.
