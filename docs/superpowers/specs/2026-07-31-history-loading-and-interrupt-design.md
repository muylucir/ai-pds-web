...
| 중단 후 표시 | **말풍선 아래 "중단됨" 한 줄** | 지금 보고 있는 턴에서 바로 끊긴 지점을 알 수 있다. 라이브 전용 표시라 새로고침 후에는 재현되지 않는다 |
...
라이브 전용이다 — 이 이벤트는 SSE 큐에만 있고 트랜스크립트나 session_store에는
들어가지 않는다. 중단한 그 턴에서만 보이고, 새로고침 후 히스토리를 복원하면
이 줄 없이 보통 말풍선으로 돌아온다.
