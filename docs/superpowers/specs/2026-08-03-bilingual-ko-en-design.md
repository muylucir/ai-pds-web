...
번역만으로 끝나지 않고 **깨지는** 지점 여섯 곳이다. 하나(a)는 실제 결함,
하나(c)는 기계 신호의 언어 결합, 하나(b)는 접두사 결합이고, (d)는 이미 언어
중립화 선례가 있어 손대지 않는다.
...
**(c) 중단 마커 — `status` 이벤트 텍스트로 상태를 판정한다**

`claude_driver.py:735`가 턴 중단 시 `AgentEvent(kind="status", text="중단됨")`을
큐에 넣고, `useWorkspaceStream.ts:156`이 **그 문자열을 비교해** `interrupted`
플래그를 세운다. 백엔드 문구를 번역하면 프론트가 중단을 인지하지 못한다.

(a)와 달리 이것은 **순수한 기계 신호다** — 에이전트가 읽지 않고
트랜스크립트에도 남지 않는다(라이브 SSE 큐에만 있다). 그래서 여기는 언어 중립
마커가 맞다: 백엔드가 `text="interrupted"`를 보내고 프론트가 그 값으로 판정한
뒤 UI 언어의 문구를 **자기가** 그린다. 화면에 "중단됨"이라고 쓰는 것은
프론트의 일이고, 이미 `interrupted` 플래그로 그렇게 하고 있다.

**(d) `parsers/audit.py`, `parsers/questions.py` — 손대지 않음**
