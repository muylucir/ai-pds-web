...
## 빌드 세션 수명 (2026-07-30 추가)

자동 테스트가 덮지 못하는 것은 하나다: **실제 모델이 `build_complete` 도구를
부르는가.** 도구 배선과 프롬프트 문구는 단위 테스트로 고정했지만, 에이전트가
그 도구를 호출하겠다고 판단하는지는 실행해 봐야 안다. 아래 첫 항목이 실패하면
나머지는 전부 도달할 수 없으므로 먼저 확인한다.

- [ ] 빌드를 끝까지 진행 → 에이전트가 `build_complete`를 호출한다
      (호출하지 않으면 유휴 타이머가 정리하고 다음 시작은 `resume`으로 떨어진다 —
      기능이 깨지는 것은 아니지만 이 설계의 이점이 사라지므로 프롬프트를 손봐야
      한다는 신호다)
- [ ] 완료 카드가 요약과 함께 뜬다. `remaining`을 적었으면 "남은 작업"도 보인다
- [ ] 카드에서 **[호스팅 시작]이 409 없이 성공한다** — 이 작업의 동기가 된 버그다
- [ ] 유예(약 5초) 후 목록의 `active_builds`가 줄어든다
- [ ] **완료 직후 브라우저를 새로고침해도** `active_builds`가 회수된다 —
      종료를 백엔드가 소유한다는 증거다(프론트가 닫는 방식이었다면 슬롯이 남는다)
- [ ] [개선 이어서 하기] → 이전 빌드 요약을 언급하며 무엇을 개선할지 묻는다
      (전체 트랜스크립트를 다시 읽는 기색이 없어야 한다)
- [ ] 개선 세션도 다시 `build_complete`로 끝난다
- [ ] 30분을 넘는 빌드가 진행 중에 죽지 않는다(긴 프로토타입으로 확인)
- [ ] 질문 카드를 띄운 채 30분 이상 방치한 뒤 답변 제출 → 409가 아니다
