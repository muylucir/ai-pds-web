// frontend/lib/approvalMarker.ts — 감사 로그에서 승인 결정을 알아보는 판정식.
//
// 승인 턴의 텍스트는 백엔드가 정한다(backend/aipds/routes/approval.py의
// `turn_text`). 이 판정식이 그 텍스트와 어긋나면 승인 게이트가 조용히 열리지
// 않는다: 보낸 텍스트를 감사 로그에서 되찾지 못하고, 사용자는 버튼을 눌렀는데
// 아무 일도 일어나지 않은 것으로 본다.
//
// 왜 불투명 마커(`[APPROVED]`)가 아닌가: 이 텍스트는 기계 신호가 아니다.
// 승인 턴은 트랜스크립트와 채팅 히스토리에 사용자 말풍선으로 남는다. 에이전트가
...
//
// 게이트 버튼이 도는 턴은 항상 위 두 단어 중 하나이므로 위 판정식으로 충분하다.
