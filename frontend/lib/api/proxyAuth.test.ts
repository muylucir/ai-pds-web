...
describe("withBearer", () => {
  ...
  it("forwards the prototype access cookie while still dropping session cookies", () => {
    ...
    const out = withBearer(
      new Headers({
        cookie: "pf_access=secret; pf_proto_abc123=prototoken; pf_refresh=alsosecret",
      }),
      "tok-1");
    expect(out.get("cookie")).toBe("pf_proto_abc123=prototoken");
...
describe("forwardableCookies", () => {
  it("keeps only pf_proto_* cookies", () => {
    expect(forwardableCookies("pf_proto_a=1; pf_access=2; pf_proto_b=3"))
      .toBe("pf_proto_a=1; pf_proto_b=3");
  });
  ...
  it("preserves a value containing '=' padding", () => {
    ...
    expect(forwardableCookies("pf_proto_x=abc==")).toBe("pf_proto_x=abc==");
  });
  ...
  it("does not fall for a session cookie whose VALUE mentions the prefix", () => {
    ...
    expect(forwardableCookies("pf_access=pf_proto_nope")).toBeNull();
  });

  it("tolerates whitespace between cookie pairs", () => {
    expect(forwardableCookies("pf_access=1;pf_proto_a=2;   pf_proto_b=3"))
      .toBe("pf_proto_a=2; pf_proto_b=3");
  });
});
