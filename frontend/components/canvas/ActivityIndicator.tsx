...
// 아직 도구가 하나도 실행되지 않은 구간(턴 시작 직후 모델이 생각만 하는 동안)
// 에도 표시가 있어야 한다 — 그 구간이 가장 길고 가장 불안한 구간이다.
const THINKING_LABEL = "생각하고 있어요";

export function activityLabel(tool: string | null | undefined): string {
  if (!tool) return THINKING_LABEL;
...
export function ActivityIndicator({ tool }: { tool: string | null | undefined }) {
  ...
  return (
    ...
    >
      <Spinner />
      <span className="text-xs font-medium text-violet-700">{activityLabel(tool)}</span>
