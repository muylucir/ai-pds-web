...
"""Uploaded file -> text the agent can read with file_read.

The agent has text tools only, so it cannot open an xlsx itself; we convert at
upload time into a markdown table. The result is truncated at the same 50,000
character limit the rules use for URL mode (spec §6). The content is untrusted
input -- it is only ever stored as text.
"""
...
def convert(filename: str, data: bytes) -> tuple[str, bool]:
    ...
    if ext == ".xlsx":
        ...
        try:
            ...
            raise ValueError(f"cannot parse {ext} file: corrupted or invalid") from e
    else:  # .md .txt .csv -- kept as text (lossy decode)
...
def upload_key(filename: str) -> str:
    """`uploads/{uuid8}/{original name}.{original extension}.md`.
    ...
    The Korean filenames above are kept verbatim: they are the measured case,
    and the `가-힣` class in the slug regex below exists to preserve exactly
    those characters instead of collapsing them to hyphens.
    """
    name = PurePosixPath(filename or "").name          # drop any path parts
    # `가-힣` is deliberate: Korean filenames are the common case here, and
    # \w alone would already keep them under Python 3's unicode default -- the
    # explicit class documents the intent and survives a future re.ASCII flag.
