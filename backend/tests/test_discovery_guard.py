...
# ---- StrandsDriver(롤백 경로)의 도구도 같은 경계를 본다 ----

def test_strands_file_write_refuses_outside_aiplc_docs(tmp_path):
    """실효 게이트는 ClaudeDriver의 PreToolUse 훅이지만 StrandsDriver는 롤백
    경로다 — 한쪽만 막으면 드라이버를 되돌린 순간 결함이 되살아난다.
    `_confine`은 워크스페이스 탈출만 막으므로 이 경로를 그대로 통과시켰다."""
    from pathfinder.agent.strands_tools import build_tools

    tools = {t.tool_name: t for t in build_tools(str(tmp_path), str(tmp_path),
                                                 lambda ev: None)}
    write = tools["file_write"]

    out = write(path="prototype/index.html", content="<html></html>")
    assert "거부됨" in out and "prototype/index.html" in out
    assert not (tmp_path / "prototype" / "index.html").exists(), "거부했는데 썼다"

    ok = write(path="aiplc-docs/discovery/notes.md", content="# note")
    assert "written" in ok
    assert (tmp_path / "aiplc-docs" / "discovery" / "notes.md").is_file()


def test_strands_file_append_refuses_outside_aiplc_docs(tmp_path):
    """file_write만 막으면 file_append로 그대로 나간다(둘 다 파일을 만든다)."""
    from pathfinder.agent.strands_tools import build_tools

    tools = {t.tool_name: t for t in build_tools(str(tmp_path), str(tmp_path),
                                                 lambda ev: None)}
    append = tools["file_append"]

    out = append(path="prototype/app.js", content="console.log(1)")
    assert "거부됨" in out
    assert not (tmp_path / "prototype" / "app.js").exists()

    ok = append(path="aiplc-docs/audit.md", content="entry\n")
    assert "appended" in ok
