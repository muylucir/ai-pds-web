...
async def test_a_new_turn_does_not_lose_messages_to_the_previous_reader(tmp_path):
    ...
    # 소비자가 버린 턴이 CLI에서 아직 돌고 있으면 새 턴을 **거부**한다. 새 메시지가
    # query()까지 가면 `_retire_reader()`가 진행 중인 턴의 리더를 취소하고 같은 CLI
    # 세션에 query()를 겹쳐 넣는다 — 사용자가 기다린 작업이 버려진다. 거부하면 두
    # 리더가 경쟁하는 원래 위험도 함께 사라진다: 두 번째 리더가 만들어지지 않는다.
...
# ---- 소비자 없이 도는 턴 ----
# 턴의 소비자(턴 작업, aipds/turn_job.py)가 취소되어도 CLI 턴은 계속 돌고 리더도
# 계속 읽는다. 그 상태에서 새 턴은 거부되므로(has_live_turn), 파킹된 질문은 여전히
# 다시 띄울 수 있어야 하고 그 턴은 끊을 수 있어야 한다 — 아니면 사용자가 막힌다.
