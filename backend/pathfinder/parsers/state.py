...
def normalize_stage_name(raw: str) -> str:
    ...
    **왜 필요한가(2026-08-18 실측: hpt-sarang).** 모델이 `report_stage`에
    ...
    2. `upsert_stage`와 아래 `parse_state_file`의 이름 매칭이 어긋난다 —
       `&amp;`가 박힌 줄은 다음의 **올바른** 호출과 이름이 다르므로 체크라인이
       하나 더 생기고, 진행률이 같은 스테이지를 두 번 센다.
    3. 부분 포함 폴백(`_names_match`)이 엉뚱한 줄을 최장 일치로 고를 수 있다.

    **쓰기와 읽기 양쪽에서 부른다.** 쓰기(`agent/tools.py`의 `report_stage`)만
    고치면 이미 `&amp;`가 박혀 저장된 파일은 영구히 그렇게 보인다 — hpt-sarang의
    `aiplc-state.md`가 지금 그 상태다. 읽기 쪽 정규화가 그 파일들을 고치지 않고
    치유한다.
