...
def build_app(driver, workspace: str) -> Starlette:
    ...
    async def answers(request):
        ...
        # failure. Validate up front and return a normal 400 instead. (In the
        # SDK model answers don't open their own stream -- they feed the
        # already-open /message stream -- so this handler is a plain 204, but
        # the up-front validation lesson still applies to any driver call.)
        try:
